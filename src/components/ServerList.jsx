...
export default function ServerList({ selectedServer, setSelectedServer }) {
    ...
    const [isInviteErrorModalOpen, setIsInviteErrorModalOpen] = useState(false);
    ...
    const submitJoinServer = async () => {
        ...
        try {
            ...
            if (!res.ok) {
                ...
                setIsInviteErrorModalOpen(true);
                return;
            }
            const data = await res.json();
    ...
    return (
        <>
            ...
            {isInviteErrorModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
                    <div className="w-[90%] max-w-sm rounded-md bg-[#2B2D31] border border-[#1E1F22] p-5">
                        <h3 className="text-white text-lg font-semibold">Invalid Invite Code</h3>
                        <p className="text-[#B5BAC1] text-sm mt-2">Please check the code and try again.</p>
                        <div className="mt-4 flex justify-end">
                            <button
                                type="button"
                                onClick={() => setIsInviteErrorModalOpen(false)}
            ...
            {isCreateServerModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
                    <div className="w-[90%] max-w-sm rounded-md bg-[#2B2D31] border border-[#1E1F22] p-5">
                        <h3 className="text-white text-lg font-semibold">Create Server</h3>
                        <p className="text-[#B5BAC1] text-sm mt-2">Enter server name</p>
                        <input
                            type="text"
                            value={newServerName}
            ...
            {isJoinServerModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
                    <div className="w-[90%] max-w-sm rounded-md bg-[#2B2D31] border border-[#1E1F22] p-5">
                        <h3 className="text-white text-lg font-semibold">Join Server</h3>
                        <p className="text-[#B5BAC1] text-sm mt-2">Enter invite code</p>
                        <input
                            type="text"
